(async () => {
  ...
  fs.readdirSync(
    path.join(__dirname, "../Docker/Volumes/maro-api/backup")
    ...
    const Schema = require(path.join(
      __dirname,
      "../Docker/Volumes/maro-api/storage/schemas/",
      path.basename(file, ".csv")
    )).schema;
    const Model = mongoose.model(
      file == "leaderboard" ? "leaderboards" : path.basename(file, ".csv"),
    ...
    csv()
      .fromFile(
        path.join(
          __dirname,
          "../Docker/Volumes/maro-api/backup",
          file == "leaderboard.csv" ? "leaderboards.csv" : path.basename(file)
        )
      )
      .then((jsonObj) => {
        Model.find(
          ...
          (err, docs) => {
            if (err) return console.log(err);
              ...
              let toAdd = jsonObj.filter(
                (e) => !docs.find((e2) => e2._id == e._id)
              );
              if (toAdd > 0) {
                Model.insertMany(toAdd, (err, docs) => {
                  if (err) console.log(err);
                    ...
                    console.log(
                      `Successfully imported ${toAdd.length} ${file}`
                    );
                  total += toAdd.length;
                  i++;
                });
