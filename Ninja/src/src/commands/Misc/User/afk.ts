module.exports = {
    ...
    async execute(message: any, args: Array<string>, client: any, paguClient: any) {
        ...
        await userSchema.findOne({ userID: message.author.id }, async (err: Error, data: any) => {
            var afk = new Boolean;
            if (err) {
                console.log(err)
            } else if (data) {
                afk = !data.afk.afk
                await paguClient.Util.createNewUser(paguClient, message, undefined, { reason: args.length==0?"No reason specified":args.join(' '), afk: !data.afk.afk })
            } else if (!data) {
                afk = true
                await paguClient.Util.createNewUser(paguClient, message, undefined, { reason: args.join(' '), afk: true })
            }
            if(afk) {
                if (args.length == 0) {
