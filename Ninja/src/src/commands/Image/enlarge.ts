...
module.exports = {
    ...
    async execute(message:Message, args:Array<string>, client:Client, paguClient:any) {
        ...
        let id = (isNaN(parseInt(args[0]))) ? ((args[0].includes(":")) ? args[0].slice(args[0].lastIndexOf(":") + 1, -1) : args[0]): args[0]
        fs.readdir( '/application/attachments/72x72/', (err:any, files:Array<any>) => {
            let splitEmoji = [...id]
            ...
            if (files.includes(filename)) {
                message.channel.send({files: [`/application/attachments/72x72/${filename}`]})
                return
            }
            else {
                const emoji_url_gif = `https://cdn.discordapp.com/emojis/${id}.gif`

                axios.get(emoji_url_gif).then((res: { status: number })=>{
