    ...
    try {
        async function registerSchemas(path: string, sub?:string) {
           fs.readdirSync(path).forEach((file: string) => {
                ...
                if(sub) paguClient.schemas[sub].set(Path.basename(file, Path.extname(file)), schema)
                else paguClient.schemas.set(Path.basename(file, Path.extname(file)),schema)
            });
        }
