          ...
          if (commandFile.name) {
            if (commandFile.name[0]) {
              paguClient.commands.set(file, {
                ...
                cache: { category: category, loaded: true, name: file },
              });
              options.client.on("messageCreate", async (message: any) => {
                for (const alias of commandFile.name) {
                  ...
                  if (
                    (args[0].toLowerCase() == `${process.env.PREFIX}${alias}` ||
                    ...
                    if (
                      ...
                      async function checkCooldown() {
                        ...
                        if (
                          paguClient.cache.internal.cooldowns[
                            commandFile.name[0]
                          ] > Date.now()
                        ) {
                        } else {
            ...
            paguClient.commands.set(file, {
              commandFile: commandFile,
              cache: { category: category, loaded: false, name: file },
            });
            paguClient.Util.log(
              __filename,
              "warn",
