...
module.exports = {
    ...
    async execute(client:any, paguClient:any, oldMessage:Message, newMessage:Message) {
        ...
        const guildData = await guildSchema.findOne({ guildID: oldMessage.guild?.id });
        const mainLoggingChannel = guildData.mainLoggingChannel
        const embed = new MessageEmbed
        embed.setAuthor({name:`${oldMessage.author.tag}`, iconURL:`${oldMessage.author.avatarURL({dynamic:true})}`})
        ...
        await mainLoggingChannel.send({embeds:[embed], attachments:attachments})
