        ...
        if (message.channel.id == "899227365228576841") {
            if (message.content.includes('http')) {
                ...
                const filter = (reaction: any, user: any) => {
                    return ['✅', '❌', '📝'].includes(reaction.emoji.name) && user !== client.user && reaction.message == msgSent
                }

                const collector = msgSent.createReactionCollector({filter, time: 21600000 })
                ...
                try {
                    await message.author.send(`You cannot speak in <#${message.channel.id}>`)
                } catch {
