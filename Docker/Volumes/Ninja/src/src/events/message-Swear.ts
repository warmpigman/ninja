      ...
      if (message.channel.id !== guildData.regularChannel.id) {
        if (
          badWords
            ...
            .some((word: String) =>
              message.content.toLowerCase().includes(word.toString())
            )
        ) {
          message.delete();
          message.channel.send(
            `${message.author.tag} has sent a very bad message. :angry:`
          );
        }
      } else if (
        badWords
        ...
        message.delete();
        message.channel.send(
          `${message.author.tag} has sent a possibly bad message. :angry:`
        );
      }
    }
  },
