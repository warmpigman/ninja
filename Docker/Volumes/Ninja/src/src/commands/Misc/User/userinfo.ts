...
module.exports = {
    ...
    async execute(
      ...
      embed.setTimestamp()
      embed.setFooter(`Requested by ${message.author.username} | ${message.author.id}`, `${message.author.avatarURL({dynamic:true})}`)

      let member_id = (args.length > 0) ? ((isNaN(parseInt(args[0]))) ? args[0].slice(3, -1) : args[0]) : message.author.id
      ...
      if (user === undefined) {
        ...
        embed.addFields([
          {name:`User Info for ${user.username}`, value:`UserID: ${user.id}`, inline:true},
          {name:`Created At`, value:`${user.createdAt.toLocaleDateString("en-GB")}, at ${user.createdAt.toLocaleTimeString("en-GB")}`, inline:true}
        ])
        if (!(member === undefined)) {
          let permissionArr = member?.permissions.toArray()
          let permissionText = ``
          for (let i = 0; i < Math.min(permissionArr.length, 10); i++) {
            permissionText += capitalizeFirstLetter(`${permissionArr.at(i)?.replaceAll("_", " ")}`) + ", "
          }
          if (permissionText === "") {
            permissionText = "None"
          } else {
          ...
          if (!(member.premiumSince === null)){
            premiumText += `${member?.premiumSince?.toLocaleDateString("en-GB")}, at ${member?.premiumSince?.toLocaleTimeString("en-GB")}`
          }
          else {
            premiumText += "Not Boosting"
          }
          embed.addFields([
            {name:`Joined Server At`, value:`${member?.joinedAt?.toLocaleDateString("en-GB")}, at ${member?.joinedAt?.toLocaleTimeString("en-GB")}`, inline:true},
