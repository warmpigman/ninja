module.exports = {
    ...
    examples: [
        "settradebanrole @tradeban",
        "settradebanrole 212148329245020082",
    ],
        ...
        guildSchema.findOne({
            ...
            if(err) {
                ...
                if(!data) {
                    ...
                    return message.reply({ content: `<@&${roleID}> has been set as the tradeban role.`, allowedMentions: {} })
                } else if(data) {
                    if(data.tradeBanRole.ID == roleID) {
                        return message.reply({ content: `<@&${roleID}> is already the tradeban role.` })
                    } else {
                        data.tradeBanRole.ID = roleID
                        data.tradeBanRole.Set = true
                        data.save()
                        return message.reply({ content: `<@&${roleID}> has been set as the tradeban role.` })
                    }
                }
            }
        })
