module.exports = {
    name: ["setmutedrole", "set-muted-role","set-mutedrole"],
    category: "Mod",
    description: "Set's the guild's muted role to mute people with.",
    usage: "setmuterequestrole [ID or #role]",
    examples: [
        "setmutedrole @muted",
        "setmutedrole 212148329245020082",
    ],
        ...
        guildSchema.findOne({
            ...
            if(err) {
                ...
                if(!data) {
                    ...
                    return message.reply({ content: `<@&${roleID}> has been set as the muted role.`, allowedMentions: {} })
                } else if(data) {
                    if(data.mutedRole.ID == roleID) {
                        return message.reply({ content: `<@&${roleID}> is already the muted role.` })
                    } else {
                        data.mutedRole.ID = roleID
                        data.mutedRole.Set = true
                        data.save()
                        return message.reply({ content: `<@&${roleID}> has been set as the muted role.` })
                    }
                }
            }
        })
