module.exports = {
    ...
    examples: [
        ...
        "setverifiedrole 212148329245020082",
    ],
        ...
        guildSchema.findOne({
            ...
            if (err) {
                ...
                if (!data) {
                    ...
                    return message.reply({ content: `<@&${roleID}> has been set as the verified role.`, allowedMentions: {} })
                } else if (data) {
                    if (data.verifiedRole.ID == roleID) {
                        return message.reply({ content: `<@&${roleID}> is already the verified role.` })
                    } else {
                        data.verifiedRole.ID = roleID
                        data.verifiedRole.Set = true
                        data.save()
                        return message.reply({ content: `<@&${roleID}> has been set as the verified role.` })
                    }
                }
            }
        })
