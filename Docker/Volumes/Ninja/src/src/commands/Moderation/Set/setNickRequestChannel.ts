module.exports = {
  name: [
    "setnickrequestchannel",
    "set-nick-request-channel",
    "set-nickrequestchannel",
  ],
  category: "Mod",
      ...
      (err: Error, data: any) => {
        if (err) {
          ...
          if (!data) {
            ...
            return message.reply({
              content: `<#${channelID}> has been set as the nick request channel.`,
            });
          } else if (data) {
            if (data.nickRequestChannel.ID == channelID) {
              return message.reply({
                content: `<#${channelID}> is already the nick request channel.`,
              });
            } else {
