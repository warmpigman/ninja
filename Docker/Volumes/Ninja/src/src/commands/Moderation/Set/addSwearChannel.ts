module.exports = {
  ...
  category: "Mod",
  description:
    "Add's a channel to the swear-allowed channels. Categories are also supported!",
        ...
        async (err: Error, data: any) => {
          if (err) {
            ...
            await guildSchema.create({
              ...
              "GUILD_CATEGORY"
                ? "swearAllowedCategories"
                : "swearAllowedChannels"]: [channelID],
            });
            return message.reply({
              content: `<#${channelID}> has been added to the swear allowed ${
                message.guild.channels.cache.get(channelID).type ==
                "GUILD_CATEGORY"
                  ? "categories"
                  : "channels"
            ...
            if (
              ...
              return message.reply({
                content: `<#${channelID}> is already a swear allowed ${
                  message.guild.channels.cache.get(channelID).type ==
                  "GUILD_CATEGORY"
                    ? "categories"
                    : "channels"
