module.exports = {
  ...
  category: "Mod",
  description:
    "Add's a channel to the spam-allowed channels. Categories are also supported!",
  usage: "addspamchannel [ID or #channel]",
  examples: ["addspamchannel #no-filter", "addspamchannel 212148329245020082"],
        ...
        async (err: Error, data: any) => {
          if (err) {
            ...
            await guildSchema.create({
              guildID: message.guild.id,
              [message.guild.channels.cache.get(channelID).type ==
              "GUILD_CATEGORY"
                ? "spamAllowedCategories"
                : "spamAllowedChannels"]: [channelID],
            });
            return message.reply({
              content: `<#${channelID}> has been added to the spam allowed ${
                message.guild.channels.cache.get(channelID).type ==
                "GUILD_CATEGORY"
                  ? "categories"
                  : "channels"
            ...
            if (
              ...
              return message.reply({
                content: `<#${channelID}> is already a spam allowed ${
                  message.guild.channels.cache.get(channelID).type ==
                  "GUILD_CATEGORY"
                    ? "categories"
                    : "channels"
