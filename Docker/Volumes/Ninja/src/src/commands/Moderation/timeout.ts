...
module.exports = {
  ...
  category: "Mod",
  description:
    "Times out a user. Valid time multipliers are seconds (s), minutes (m), hours (h), and days (d).",
  ...
  async execute(
    ...
    if (args.length == 0) {
      message.channel.send(
        `Please follow the proper usage of ${module.exports.usage}`
      );
      paguClient.commands
        .get("help")
        .commandFile.execute(message, ["timeout"], client, paguClient);
      ...
      let duration =
        args[1].match(/^\d+$/) == null
          ? isNaN(parseInt(args[1].slice(0, -1)))
            ? 0
            : Object.keys(multipliers).includes(args[1].slice(-1))
            ? multipliers[args[1].slice(-1)]
            : 0
          : parseInt(args[1]);
        ...
        if (message.guildId === null) {
          ...
          if (member === undefined) {
            ...
            member
              ...
              .then((res: any) => {
                ...
                guildSchema.findOne(
                  ...
                  async (err: Error, data: { mainLoggingChannel: string }) => {
                    ...
                    embed.addFields([
                      { name: "User Timed Out", value: `<@${member?.id}>` },
              ...
              .catch((err: any) => {
                message.channel.send(
                  "The duration specified was too long. It must be at most 4 weeks."
                );
              });
          }
        }
