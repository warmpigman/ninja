...
module.exports = {
    ...
    async execute(message:Message, args: Array<string>, client:Client, paguClient:any) {
        if (args.length == 0) {
            ...
            paguClient.commands.get('help').commandFile.execute(message, ["untimeout"], client, paguClient)
        }
        else {
            if (message.guildId === null) {
                ...
                if (member === undefined) {
                    ...
                    member.edit({communicationDisabledUntil:timestamp.toISOString()}, reason).then((res:any) => {
                        ...
                        guildSchema.findOne({guildID:message.guildId}, async (err:Error, data:{mainLoggingChannel:string}) => {
                            ...
                            embed.addFields([{name: "User Untimed Out", value:`<@${member?.id}>`},
