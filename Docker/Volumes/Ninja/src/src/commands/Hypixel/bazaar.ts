...
module.exports = {
  name: ["bz", "bazaar"],
  category: "Hypixel",
  description: "Shows the bazaar stats of an item.",
  usage: "bz [item]",
  examples: ["bz enchanted diamond", "bazaar enchanted oak"],
  async execute(
    message: Message,
    args: Array<string>,
    client: Client,
    ...
    if (args.length == 0) {
      ...
      for (const property in items) {
        ...
        if (
          ...
          embed.addFields([
            ...
            {
              name: `Buy Volume`,
              value: `${abbreviateNumber(item.buyVolume)}`,
              inline: true,
            },
            ...
            {
              name: `Sell Volume`,
              value: `${abbreviateNumber(item.sellVolume)}`,
              inline: true,
            },
          ]);
          m.edit({
      ...
      if (!found) {
        m.edit({
          content: `Could not find ${search_term}`,
          allowedMentions: {
            repliedUser: false,
            roles: [],
            users: [],
